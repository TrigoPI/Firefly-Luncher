...
import { McInstaller, ForgeInstaller } from "./McInstaller.cjs";
import { GetAppData, Get, DownloadFile } from "./Utils.cjs";
...
async function GetClientConf(): Promise<ClientConf> {
    ...
    if (err) {
        if (existsSync(`${appdir}/game.json`)) {
            const buffer: Buffer = readFileSync(`${appdir}/game.json`);
    ...
    writeFileSync(`${appdir}/game.json`, buffer);
...
async function InstallMinecraft(): Promise<void> {
    ...
    const mcInstaller: McInstaller = new McInstaller(conf.version, "windows", `${appdir}/versions/${conf.name}`);
    const mcSize: number = await mcInstaller.GetSize();
    ...
    mcInstaller.AddStepCallback((s: number) => {
        ...
        const sizeMb: number = Math.floor((mcSize / 1000000) * 10) / 10;
    ...
    await mcInstaller.InstallVersion();
}

async function InstalForge(): Promise<void> {
    const conf: ClientConf = await GetClientConf();
    const forgeInstaller: ForgeInstaller = new ForgeInstaller(conf.version, conf.forge, `${appdir}/versions/${conf.name}`);
    const forgeSize: number = await forgeInstaller.GetSize();        
    const processor: number = await forgeInstaller.GetNumberOfClientProcessor();
    ...
    forgeInstaller.AddStepCallback((s: number, step: string) => {
        ...
        if (step == "processor") {
            ...
            const display: string = `Processor : ${s} / ${processor}`;
            const step: number = 2;
    ...
    await forgeInstaller.InstallVersion();
}
...
async function InstallMods(): Promise<void> {
    ...
    if (existsSync(modsPath)) await rm(modsPath, { recursive: true });
